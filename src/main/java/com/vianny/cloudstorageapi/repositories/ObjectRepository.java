...
public interface ObjectRepository extends JpaRepository<ObjectDetails, Long> {
    ...
    @Query("SELECT new com.vianny.cloudstorageapi.dto.response.object.ObjectDetailsDTO(od.objectName, od.objectType, od.objectLocation, od.objectSize, od.uploadDate) FROM ObjectDetails od WHERE od.objectName = :objectName AND od.account.login = :login")
    ...
    @Query("SELECT new com.vianny.cloudstorageapi.dto.response.object.ObjectDetailsDTO(od.objectName, od.objectType, od.objectLocation, od.objectSize, od.uploadDate) " +
    ...
    @Query("SELECT new com.vianny.cloudstorageapi.dto.response.object.ObjectInfoMiniDTO(od.objectName, od.objectType) FROM ObjectDetails od WHERE od.objectLocation = :objectLocation AND od.account.login = :login")
    List<ObjectInfoMiniDTO> getObjectsNameByObjectLocation(@Param("objectLocation") String objectLocation, @Param("login") String login);
}
